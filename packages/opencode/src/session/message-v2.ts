...
export namespace MessageV2 {
  ...
  /**
   * Load only messages created after a given timestamp.
   * Used for incremental loading in the agent loop to avoid re-reading all messages each iteration.
   */
  export async function loadSince(sessionID: string, afterTimeCreated: number): Promise<WithParts[]> {
    const rows = Database.use((db) =>
      db
        .select()
        .from(MessageTable)
        .where(and(eq(MessageTable.session_id, sessionID), gt(MessageTable.time_created, afterTimeCreated)))
        .orderBy(desc(MessageTable.time_created))
        .all(),
    )
